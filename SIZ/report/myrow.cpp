...
myRow::myRow(const QList<QVariant> rec, QObject *parent) : QObject(parent)
{
    ...
    for (int c = 0 ; c < rec.count() ; c++){
        ...
        if (c == 10){
            if (rec.at(10).toDate().day() > 29){ //если день выдачи больше 29, то ставим 28. все изза февраля
                cells.append(new myCell(QDate(rec.at(10).toDate().year(),rec.at(10).toDate().month(), 28), this));
                continue;
            }
...
{
    if(cells.at(10)->getValue<QString>() == "Декрет") return;
...
void myRow::onceGivenRow()
{
    RESULT->addEntry("Все",
                     ...
                     cells.at(10)->getValue().toString() == "Не выдано" ? 1 : 0, //количество
